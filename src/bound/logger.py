...
r"""
    ...
    Provides utilities to simplify and standardize logging in particular for training with
    \p torch.
...
class TrainingLogger:
    ...
    def __init__(self, fld_names: List[str], fld_loss_names: List[str],
        ...
        for i in range(0, len(fld_widths) - 1, self.FIELDS_PER_MODEL):  # Shave off the time field
            # if i % self.FIELDS_PER_MODEL == 0:
            combo_width = sum(fld_widths[i + j] for j in range(self.FIELDS_PER_MODEL))
            combined_widths.append(combo_width)
